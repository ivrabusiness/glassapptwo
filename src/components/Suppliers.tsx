      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead className="bg-gray-50">
            <tr>
              {['Dobavljač', 'Tip', 'OIB', 'Adresa', 'Kontakt', 'Akcije'].map((header) => (
...
const Suppliers: React.FC = () => {
  ...
  const filteredSuppliers = suppliers.filter(supplier =>
    ...
    supplier.oib.includes(searchTerm) ||
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600">Upravljanje dobavljačima i partnerima</p>
        </div>
        <button
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                  ...
                  <tr key={supplier.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        {supplier.type === 'company' ? (
                          <Building className="h-5 w-5 text-blue-500 mr-3 flex-shrink-0" />
                        ) : (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        ...
                        {supplier.type === 'company' ? 'Tvrtka' : 'Fizička osoba'}
                      </span>
                    </td>
          ...
          {filteredSuppliers.length === 0 && !isLoading && (
            <div className="text-center py-12">
              ...
              <h3 className="mt-2 text-sm font-medium text-gray-900">Nema dobavljača</h3>
              <p className="mt-1 text-sm text-gray-500">
                {searchTerm ? 'Nema dobavljača koji odgovaraju pretraživanju.' : 'Počnite dodavanjem novog dobavljača.'}
              </p>
            </div>
          )}
        </div>
          ...
          <div className="bg-white rounded-lg max-w-md w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-200">
              <h2 className="text-lg font-medium text-gray-900">
                {editingSupplier ? 'Uredi dobavljača' : 'Dodaj novog dobavljača'}
              </h2>
            </div>
